...
|--------|------|---------|--------|-----------|
| Shruti Rajagopal | 2026-04-02 | 2.0 | Draft | Shruti Rajagopal, Shayaun Nejad |
...
**The bet:** Memory doesn't enable perception. Memory enables *standing* — the ability to hold a position grounded in months of evidence the user has forgotten or revised. Liminal Space is a structured confrontation engine for people in psychological transition.
...
**Tagline:** *Memory for who you're becoming.*
...
The current concept stores psychological data, retrieves it over time, and reflects it back as narrative text. The memory toggle in the POC shows: without memory = a number, with memory = a number plus a paragraph.
...
### The Core Interaction: Claim → Response → Correction (The Flywheel)
...
<!-- PRESENTER NOTE:
This is where you switch to the POC. Show Day 1 → Day 90 transition.
Then come back to the doc for Sections 2-4 context (2 minutes).
Then jump to Section 8 (metrics + guardrails) to close with the rigor framing.
-->
...
### Flow 2: The Confrontation (The Atomic Unit of Value)
...
1. **The assertion.** One sentence. A position, not an observation. "You're in the same configuration as January 14."
...
| Metric | Value | Source |
|--------|-------|--------|
| Hackathon | Memory Genesis, Apr 4, Computer History Museum | Event |
| Demo mode | `?demo=true` hardcoded response, zero external dependencies | POC |
| Seeded MemCells | 7 across 3 phases | `scripts/seed-demo-data.ts` |
| EverMemOS channels | 3: coherence, council, threshold | Integration map |
| EventBus handler | 16th handler, fire-and-forget | `src/services/events/handlers/EverMemOSHandler.ts` |
| Coherence factors | 7 (1-10 scoring) | Schema |
| Archetypes | 12, base set 4 available Day 1 | `src/lib/archetypes/voices.ts` |

---

## Appendix: Hackathon Demo Implications
...
## Appendix: Existing Code Mapping
